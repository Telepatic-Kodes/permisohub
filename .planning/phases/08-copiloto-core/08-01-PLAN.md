...
import { BASE as INTELIGENCIA_DOM } from '@/lib/inteligencia-dom'
...
function buildObservacionesPrompt(p: Proyecto): string {
  ...
  const intel = INTELIGENCIA_DOM.find(m => m.municipio === p.municipio)
...
function buildEstimacionPrompt(p: Proyecto, derechosInfo: string): string {
  const stats = ESTADISTICAS_MUNICIPIOS.find(m => m.nombre === p.municipio)
  const plazoBase = stats?.plazoTipicoDias ?? 45
...
export async function POST(request: Request) {
  ...
  const tipoObra = (['obra_nueva', 'ampliacion', 'alteracion', 'reconstruccion', 'regularizacion', 'demolicion'].includes(p.tipo)
    ? p.tipo
    : 'obra_nueva') as TipoObra
