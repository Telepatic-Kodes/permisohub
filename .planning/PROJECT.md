...
- **Deuda técnica conocida (no bloqueante)**: el fallback `process.env.NEXT_PUBLIC_APP_URL ?? 'http://localhost:7891'`, repetido en ~14 archivos (todo self-fetch server-side: `lib/terrenos-server.ts`, `lib/zonificacion-server.ts`, `lib/sii-lookup-server.ts`, copiloto, cron/daily-check, etc.), no coincide con el puerto real de dev documentado en `.env.local.example` (3000) — si `.env.local` no tiene la variable seteada, todo self-fetch server-side falla en silencio con connection-refused, indistinguible de un error de negocio real. Descubierto 04-08 al verificar en vivo el reintento masivo de zonificación de terrenos: 24 terrenos quedaron mal marcados `zona_status='error'` por esto, no por dirección inválida — revertidos a `pendiente`. Pendiente decidir si corregir el fallback en los 14 archivos a `localhost:3000`.
