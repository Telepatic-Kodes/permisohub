# PermisoHub

## What This Is

PermisoHub es una plataforma SaaS B2B para arquitectos chilenos que gestiona permisos de edificación municipales de principio a fin. Combina gestión de expedientes, CRM, portal cliente, automatizaciones y un copiloto de IA entrenado en la normativa chilena (OGUC, LGUC, Ley 21.718).

## Core Value

El copiloto IA del arquitecto chileno — reduce el tiempo de tramitación DOM de 124 días promedio a menos de 60, automatizando documentación, respuestas a observaciones y seguimiento.

## Tech Stack

- **Frontend**: Next.js 16.2.9 (Turbopack/Webpack), TypeScript estricto, Tailwind CSS, shadcn/ui
- **Backend**: Next.js API Routes, Supabase (PostgreSQL + Auth + RLS), Supabase MCP para migraciones/queries en sesiones de agente
- **IA**: OpenAI GPT-4o vía `lib/ai.ts` (chat, extracción PDF, verificación, comunicaciones, clasificación de compatibilidad de uso PRC) — `@anthropic-ai/sdk` instalado pero dormante, no migrar a mitad de milestone
- **Mapas**: Leaflet + OpenStreetMap raster tiles (v1.4, componente presentacional para confirmación visual de polígono de zona)
- **Datos geoespaciales**: ArcGIS FeatureServer (MINVU/OCUC), Nominatim (geocoding, sin API key)
- **Automatización**: Twilio (WhatsApp), Resend (email), Vercel Cron Jobs
- **Hosting**: Vercel (edge functions, auto-scaling)
- **Billing**: Stripe (v1.1+)

## Requirements

### Validated (v1.0 — MVP shipped Jun 2026)

- ✓ Gestión de proyectos y expedientes — v1.0
- ✓ CRM de prospectos y clientes — v1.0
- ✓ Portal cliente con seguimiento — v1.0
- ✓ Wizard de ingreso DOM en Línea — v1.0
- ✓ Chat OGUC con IA streaming — v1.0
- ✓ Extractor PDF observaciones → respuestas automáticas — v1.0
- ✓ Verificador cumplimiento OGUC pre-ingreso — v1.0
- ✓ Generador de comunicaciones formales — v1.0
- ✓ Contador días hábiles Ley 21.718 — v1.0
- ✓ Calculadora derechos municipales — v1.0
- ✓ Inteligencia de municipios — v1.0
- ✓ WhatsApp automático (Twilio) — v1.0
- ✓ Cron jobs automatizados — v1.0
- ✓ Pitch deck + Modelo de negocio (/documentos) — v1.0

- ✓ BILL-01→07: Stripe billing con 3 tiers (Starter $29.990 / Pro $79.990 / Estudio $149.990 CLP) — v1.1
- ✓ GATE-01→06: Feature gating por tier (límites de proyectos, AI chats, PDF extractions) — v1.1
- ✓ LAND-01→04: Landing page pública con pricing y SEO — v1.1
- ✓ ONBD-01→03: Onboarding wizard 3 pasos post-signup — v1.1
- ✓ PWA-01→03: App instalable en móvil — v1.1
- ✓ Dashboard Timeline View (4 secciones de urgencia, hero stats) — v1.2
- ✓ Copiloto IA drawer en Permisos, Desarchivo y Patentes — v1.3
- ✓ Verificación DOM diaria automática (write-back sin click del arquitecto) — v1.3
- ✓ Enriquecimiento SII automático al crear patente comercial — v1.3
- ✓ Resumen semanal por email con tip de IA — v1.3

- ✓ Zonificación automática por dirección (zona PRC + código/nombre + usos permitidos/prohibidos vía ArcGIS FeatureServer MINVU/OCUC, geocoding Nominatim, caché compartida por coordenadas) — v1.4
- ✓ Mapa con confirmación visual de polígono de zona (Leaflet + OSM) — v1.4
- ✓ Verificación de compatibilidad de uso (uso pretendido vs. usos permitidos/prohibidos, clasificación IA de 3 estados — Permitido/No permitido/No especificado, nunca binario) — v1.4
- ✓ Citación a fuente oficial (link a decreto cuando existe) + disclaimer CIP siempre visible — v1.4
- ✓ Fallback manual de comuna/zona cuando falla el geocoding o no hay cobertura — v1.4
- ✓ Acción explícita "Actualizar" (sin refresco silencioso) — v1.4
- ✓ Cobertura inicial: Las Condes, Providencia, Vitacura, Ñuñoa — v1.4
- ✓ Integración aditiva de la zona en vía de tramitación (alerta citada), due diligence (cita PRC), y copiloto IA (contexto en diagnóstico OGUC + checklist) — sin alterar la lógica determinista de `recomendarVia()` — v1.4

### Requirements (Torre de Control — shipped 2026-08-01, mergeado a main)

**Torre de Control** — gobernanza de datos y decisiones (ver `.planning/data-sources.yaml`), motivada porque el milestone v1.5 (fusión PROPRA·BI) se ejecutó en sesiones ad-hoc sin actualizar `.planning/`, dejando 17 commits invisibles a este sistema hasta esta puesta al día.

- ✓ Registro de fuentes de datos (`data-sources.yaml`, 32 fuentes) + validador determinista (`scripts/check-data-sources.mjs`) + `reportError`/`reportWarning` en los 7 scrapers que solo hacían `console.warn`
- ✓ Tabla `data_source_runs` + `recordSourceRun()` + página `/admin/salud-datos` — migración `20260807_data_source_runs.sql` aplicada en Supabase (confirmado en vivo: tabla con datos reales de mercado-locales/instrumentos-ipt/terrenos/cadenas-sucursales)
- ✓ Git hook de captura automática de commits (`.githooks/post-commit`) + puesta al día de `.planning/`
- ✓ Comando `/torre-control` project-local
- ✓ `InformeEjecutivo` piloteado en Tasación y extendido a Due Diligence — decisión explícita de NO extender a Predictor/Auditor/Compliance-check/Reportes de Mercado/Memoria Descriptiva (ya tienen UI a medida equivalente o el género no aplica, ver Key Decisions)
- ✓ 3 decisiones de founder resueltas (TocToc pendiente sin bloquear, Unimarc sin más inversión, refresh legal en cadencia fija trimestral)

### Requirements (v1.5 — Fusión PROPRA·BI, shipped 2026-08-01, ver `.planning/MILESTONES.md`)

- ✓ Módulo Terrenos (descubrimiento + evaluación) — v1.5
- ✓ Mercado Inmobiliario (tasación, pricing, oportunidades, due diligence, calculadora de inversión, reportes, indicadores macro, noticias) — v1.5
- ✓ Copiloto conversacional de Mercado Inmobiliario (function/tool-calling) — v1.5
- ✓ Instrumentos IPT por comuna — v1.5
- ✓ Checklist dinámico de requisitos DOM (fuente real, 9 formularios MINVU) — v1.5
- ✓ Separación de módulos Permisos/Mercado Inmobiliario en la UI (switcher, sidebar contextual, badge) — v1.5
- ✓ Fusión de permisohub-enterprise en una sola app — v1.5

### Requirements (auditoría de código + Mi Cartera + cierre de deuda — shipped 2026-08-02, mergeado a main)

Serie de sesiones ad-hoc entre v1.5 y v1.6 (sin milestone formal propio) — capturado acá para que "Validated" refleje la realidad del código:

- ✓ Auditoría de código de Mi Cartera, resto de Mercado Inmobiliario, y Permisos/DOM — 1 falla P0 de seguridad (RLS cross-tenant, patrón encontrado 3 veces), workspace-sharing roto en 4 rutas, fabricación financiera en cuadros de cálculo/derechos municipales, honestidad de IA (fabricación en fallos de parseo, truncamiento silencioso), SSE roto, N+1, timezone UTC — ver bullets `[auditoria-*]` en `.planning/STATE.md` Accumulated Context
- ✓ Mi Cartera: alerta de reajuste de renta UF/IPC (Ley 18.101 Art. 13) + Cap Rate en el resumen de portafolio — primer uso de 2 agentes en paralelo (git worktrees) en el proyecto
- ✓ Torre de Control: cierre de 5/6 `needs-decision` del registro de fuentes (spot-check real de 23 comunas, decisión de no cablear RAG-embeddings, cierre de DOM Digital tras investigar viabilidad)

### Requirements (v1.6 — Reportes Profesionales de Oportunidades, shipped 2026-08-02)

- ✓ DETA-01→07: Dashboard de detalle por oportunidad individual (`/oportunidades/[id]`) — posicionamiento vs. cohorte con `muestra_n` explícito, historial de precio, reason codes + señales cruzadas, comparables sugeridos, resumen ejecutivo IA bajo demanda, rentabilidad implícita de zona — v1.6
- ✓ COMPA-01→04: Comparación lado a lado de 2-5 oportunidades homogéneas, tabla con mejor valor resaltado, prevención estructural real (server-side, no solo checkbox) de mezclar tipo/operación, estado persistido en URL — v1.6
- ✓ INFO-01→04: Informe exportable/imprimible (individual y de comparación) vía `@media print`, con fecha de generación Y de última verificación por dato, campo "preparado por/para" personalizable — v1.6
- ✓ `evaluarOportunidad()` extraído como fuente única de verdad para scoring (TDD) — previene divergencia entre lista y ficha de detalle
- ✓ Patrón `@media print` + `window.print()` establecido como estándar del proyecto para vistas exportables (sin jsPDF/html2canvas) — resuelve una contradicción interna del research de milestone entre sus propios documentos
- ✓ (04-08, fuera de milestone) Dashboard de zonificación a nivel portafolio en `/proyectos/zonificacion` — reusa GET /api/proyectos y el mismo mapeo zona_status→Veredicto de `/terrenos/page.tsx`, KPIs con `Dato`, filtros por estado/municipio/búsqueda

### Backlog (no comprometido — semillas para futuras sesiones de discovery)

- [ ] Exportar PDF/anexo del hallazgo de zonificación para el expediente
- [ ] Indicador de vigencia/antigüedad del PRC ("vigente desde...") — pendiente confirmar si el layer expone fecha de decreto
- [ ] Ampliar cobertura de comunas más allá de las 31 actuales (19 de dedicada + 12 de la capa agregada PrcCuencaMaipo, 04-08) — quedan ~15 comunas RM sin cobertura; requeriría un publicador nuevo, el catálogo del owner OCUC y la capa agregada ya están exhaustados
- [ ] Coeficientes urbanísticos numéricos (FOS, constructibilidad, altura, rasante, distanciamiento) — requiere fuente de datos distinta, paga o verificada
- [ ] Aplicar el mismo tratamiento de reporte profesional (research-driven) a Reportes de Mercado, una vez validado en Oportunidades v1.6

### Out of Scope (v1.6 — resuelto, para referencia histórica)

- Cap rate/NOI real por activo — no calculable con datos de listing (un scrape es arriendo O venta, nunca ambos del mismo activo); solo se ofrece "rentabilidad implícita de zona", etiquetada como estimado
- Rent roll, Walk Score, score único de "mejor oportunidad" — contradice la disciplina de "nunca fabricar interpretación no fundamentada" del proyecto
- Boilerplate legal de Offering Memorandum institucional (formato EE.UU.) — no aplica al contexto chileno/PYME de arquitectos
- Reportes de Mercado (página separada) — fuera de alcance, solo Oportunidades; evaluar mismo tratamiento después (ver Backlog)
- Mapa de posicionamiento a nivel comuna — bloqueado, sin GeoJSON de comunas RM disponible
- jsPDF/html2canvas para generación de PDF sin usuario presente — ninguno de los requisitos lo pedía; `@media print` cubre el caso de uso interactivo

### Out of Scope (v1.4 — resuelto, para referencia histórica)

- Coeficientes urbanísticos numéricos (FOS, coef. constructibilidad, altura máxima, rasante, distanciamiento) — ningún layer público de MINVU/OCUC los expone; requeriría parsear Ordenanza Local por comuna, esfuerzo de otro milestone
- Cobertura nacional completa (345 comunas) — se amplía oportunistamente, no fue meta de v1.4
- Alianza o integración de datos con zonificación.cl — decisión explícita de no depender de terceros pudiendo usar la fuente pública (MINVU/OCUC) directamente

### Out of Scope (v1.1)

- DOM en Línea API integration — gobierno aún no la publica (Q4 2026+)
- Marketplace de revisores independientes — Q4 2026
- Analytics para constructoras — Q1 2027
- API pública — Q2 2027
- Expansión LATAM — 2028+

## Context

- **Mercado**: ~8.000 arquitectos activos en Chile, 7.741 permisos/año, 124 días promedio, US$2B pérdidas anuales
- **Competencia principal**: REVI (CChC+Google) solo cubre 12/346 municipios (3.5%) y sirve a la DOM, no al arquitecto
- **Marco legal**: Ley 21.718 (ene 2025) — 30 días hábiles máximo DOM. Ley Marco Autorizaciones Sectoriales (sept 2025)
- **GTM**: Colegio de Arquitectos + AOA, SEO normativa chilena, referidos $50K CLP
- **Post-v1.4**: Supabase MCP configurado a nivel `user` en `~/.claude.json` — reusable en futuras sesiones de agente sin reconfigurar (project-ref `nojejnebedjpbdlynrqs`). Dev-auth bypass (`BYPASS_AUTH=true` + `/auth/dev-login`) confirmado funcional para testing con sesión real sin password — usar en vez de reportar "sin browser/sesión disponible" como bloqueo en futuros checkpoints.
- **Deuda técnica conocida (no bloqueante)**: ninguna pendiente. (Resuelto 04-08: fallback `NEXT_PUBLIC_APP_URL ?? 'http://localhost:7891'` corregido a `localhost:3000` en los 14 archivos que lo repetían — no coincidía con el puerto real de dev documentado en `.env.local.example`, así que sin la variable seteada todo self-fetch server-side fallaba en silencio con connection-refused, indistinguible de un error de negocio real. Descubierto al verificar en vivo el reintento masivo de zonificación de terrenos: 24 terrenos quedaron mal marcados `zona_status='error'` por esto, no por dirección inválida — revertidos a `pendiente` en producción antes del fix. Resuelto 04-08: causa raíz del backlog de `zona_status='error'` en terrenos scrapeados — `geocodeComunaCentroide()` (lib/geocoding.ts) existía y estaba probada con test unitario pero nunca conectada; ahora es el fallback de `/api/zonificacion/lookup` cuando la dirección/título del aviso no geocodifica. Nueva columna `precision` en `zonificacion_cache` + `zona_precision` snapshot en `proyectos`/`terrenos` (`'exacta' | 'centroide_comuna'`) — nunca se muestra un resultado de centroide como si fuera preciso: badge "aprox." en las tablas de listado + banner amber explícito en el detalle de proyecto/terreno. Resuelto 04-08: cobertura de comunas ampliada de 19 a 31 — la capa agregada PrcCuencaMaipo (ya usada solo para Ñuñoa) en realidad cubre 47 comunas de la cuenca completa; verificado en vivo UPERM/UPROH real (no vacío/placeholder) para 12 más: Maipú, La Florida, Peñalolén, San Bernardo, Quilicura, Huechuraba, Lo Barnechea, Colina, San Ramón, Quinta Normal, La Reina y Santiago — estas dos últimas rescatadas pese a que sus capas ArcGIS DEDICADAS propias no tienen uperm/uproh (descubierto y descartado 31-jul, resuelto ahora vía la agregada). La Pintana y Peñaflor quedaron fuera: su único dato es "Ver Zonificación del PRMS", sin texto de uso real. Como bonus, `COMUNAS_CON_ZONIFICACION` (lib/scrapers/terrenos-common.ts) deriva de este mismo registro, así que los scrapers semanales ahora también descubren terrenos nuevos en las 12 comunas agregadas, no solo resuelven zonificación de los ya existentes. Resuelto 04-08 (bug real, no cosmético): al extender `RankingBarChart` al tab Posicionamiento de oportunidades se detectó y arregló un 500 — `PosicionamientoTab` era Server Component y le pasaba un `formatValor` (función) a un Client Component (`RankingBarChart`), ilegal en RSC; se marcó `"use client"`. De paso se encontró que el gauge de precio ya mostraba "0 UF" como si fuera un precio real cuando `precioValido` era `false` (o cuando era `true` pero `precioUfNormalizado` quedaba en 0 por falta de banda de conversión CLP→UF del día) — fabricaba una posición ("más barato que toda la cohorte") que no existía. Nuevo guard `precioComparable` oculta el gauge/gráfico y muestra el motivo real en texto en ambos casos. El `RankingBarChart` de P25/Mediana/P75 también cae a UF/m² cuando las bandas de UF total no alcanzan (campo nullable real en `BandasMercadoLocal`) en vez de quedar con el título flotando sin gráfico.)
- (Resuelto 04-08: reintento masivo user-facing de `ubicacion_status='pendiente'` — 427 de 703 terrenos con zona resuelta no tenían señales de Overpass (avenida/anchors). `POST /api/terrenos/reintentar-ubicacion`, mismo patrón que reintentar-pendientes pero RLS-scoped al workspace del usuario en vez de global — la ruta cron `backfill-ubicacion` sigue existiendo para el trickle global entre workspaces, no son redundantes. Lotes de 10 por el throttle real de Overpass — verificado en vivo, ~13s/ítem en promedio.)
- (Nuevo 04-08: comparador terreno-vs-local `/terrenos/[id]/comparar-local` — picker de locales activos de `mercado_locales_listings` en la misma comuna del terreno + tabla lado a lado (precio CLP/UF, precio/m², superficie, formato/tipo, uso normativo, señales de mercado, tiempo a operar). Ruta NUEVA a propósito, no reusa `/oportunidades/comparar`: su validador COMPA-03 bloquea mezclar tipo/operación, que es justo lo que este comparador hace. Sin capa normativa del lado de oportunidades (confirmado: la tabla no tiene columna de zona/uso) — la fila "Uso normativo" dice explícitamente "N/A" de ese lado en vez de omitir o inventar equivalencia. 2 endpoints nuevos: `GET /api/mercado-locales/buscar?comuna=` y `GET /api/mercado-locales/[id]`. Verificado en vivo end-to-end con datos reales de Cerro Navia.)
- (Nuevo 04-08: segunda fuente de `mercado_locales_listings` — Doomos.cl, junto a Portalinmobiliario. Validado antes de construir: 436 avisos reales solo en "arriendo locales comerciales RM" en Doomos. Reusa el mismo card-parsing HTML y los slugs de comuna del scraper de terrenos de Doomos ya existente — sin mapeo de slugs propio. `buscarLocalesComerciales()` en `lib/scrapers/doomos.ts`, 2 rutas cron nuevas (`mercado-locales-doomos`, `mercado-locales-doomos-tipos-adicionales`) agregadas a `lib/cron-dispatch.ts`. Corrida real: 2.111 avisos nuevos (452 local_comercial + 1.676 en oficina/bodega/industrial), 0 errores. Impureza de categoría conocida y documentada en el código: ~9% de los avisos de `local_comercial` en comunas de baja densidad comercial (ej. La Granja) son en realidad casas/sitios — la fuente rellena la categoría cuando no tiene suficientes locales reales; no se filtra por palabra clave (riesgo de descartar locales reales) — mismo criterio que la impureza ya tolerada de "industrial" en Portalinmobiliario. **Hallazgo colateral corregido**: el índice único de `mercado_locales_listings` era `(fuente, fuente_id)` sin `tipo_propiedad` — un mismo aviso listado en 2 categorías (confirmado en vivo: `DM-3002938` en "bodega" e "industrial" a la vez) se pisaba entre sí en el upsert. Afectaba también a Portalinmobiliario, ya en producción, no solo a la fuente nueva — corregido con `(fuente, fuente_id, tipo_propiedad)`, se auto-repara en la próxima corrida de cada fuente.)
- (Resuelto 03-08: mojibake residual de Las Condes y etiqueta de cita del fallback manual — ver checkpoint 11-08. Resuelto 04-08: el checklist del copiloto nunca persistía — el insert escribía columnas inexistentes contra `document_checklist_items`, 0 filas en prod, verificado en vivo — se corrigió el mapeo de columnas y de paso se agregó detección de staleness contra `zona_consultada_el` con botón explícito "Regenerar checklist" que preserva por `item_key` los ítems ya marcados `ok`.)
- (Nuevo 04-08: `obtenerPoblacionEnPoligono()` (censo INE 2017) y `obtenerConsumoEstimado()` (EPF 2022 + CASEN 2024) — Fase 17 de v1.7 "Cabida Comercial" — estaban terminadas y probadas desde el 03-08 pero sin ningún caller en toda la app: el plan original (17-03) las mostraba dentro de un tab "Cabida Comercial" gateado por la isócrona real de caminata/manejo (Fase 16, bloqueada por un 403 persistente de ORS/HeiGIT, pausada por decisión del usuario). Conectadas ahora a `/terrenos/[id]` vía `GET /api/demografia-consumo` + `<DemografiaConsumoCard>`, usando un círculo de radio fijo 1km (`turf.circle`) en vez de esperar la isócrona — SIEMPRE declarado como tal en la UI ("radio fijo... no una ruta de caminata o manejo real"), mismo principio de no-fabricación que `zona_precision='centroide_comuna'`. Población solo se consulta si el terreno tiene lat/lng (no todos los tienen); consumo (CASEN/EPF) se muestra siempre por comuna, con `categoriasPendientes` explícito para las categorías EPF sin dato transcrito. Verificado en vivo con Playwright en dos casos reales: Ñuñoa con lat/lng (58.270 personas / 27.743 viviendas en 168 manzanas, tasa de pobreza 5,7%) y Renca sin lat/lng (bloque de población correctamente omitido, solo consumo). Extendido el mismo día al tab "Historial" de `/mercado-inmobiliario/oportunidades/[id]` — mismo `<DemografiaConsumoCard>` sin lat/lng, ya que `mercado_locales_listings` no tiene esa columna, así que ahí solo se muestra consumo (nunca población), reusando la misma degradación graceful ya probada con Renca. Colocado junto a las otras señales de comuna ya existentes ahí (`senalExpansion`, `tendenciaConstruccion`), mismo patrón. Verificado en vivo con un local de Maipú: tasa de pobreza comunal 12% (CASEN 2024), sin bloque de población, 0 errores de consola. Nuevo 04-08 (mismo día): rediseño visual de `<DemografiaConsumoCard>` — de texto plano a las piezas reales del "Tema Consultora" ya usadas en el resto de Mercado Inmobiliario: `KpiCard` (con badge "Verificado" porque personas/viviendas/tasa de pobreza son datos transcritos reales, no estimaciones IA) para personas, viviendas y tasa de pobreza comunal, y `RankingBarChart` para las categorías EPF con dato real (nunca un donut — el comentario del propio componente lo prohíbe cuando faltan categorías, porque un donut implica que los segmentos visibles suman el 100%, y acá 9 de 12 categorías EPF siguen sin transcribir). Nombres largos de categoría (ej. "Vivienda, agua, electricidad, gas y combustibles") se abrevian solo para el eje del gráfico vía una heurística simple (primer segmento antes de la coma o del " y ") — el nombre completo se mantiene en el texto de abajo, no se pierde. Verificado en vivo en los 3 casos (Ñuñoa con población, Renca sin población, oportunidad de Maipú) — 0 errores de consola, capturas de pantalla confirman el layout.)
- (**Fase 16 DESBLOQUEADA 04-08** — el blocker de todo el milestone v1.7 "Cabida Comercial". Diagnóstico primero: la key de ORS ya presente en `.env.local` (119 chars) devuelve 403 "Access to this API has been disallowed" en los TRES endpoints (isochrones, directions, geocode) — o sea es bloqueo de cuenta, no de endpoint ni config local; importante descartarlo porque esta misma mañana un fallo "externo" resultó ser el puerto `7891` mal configurado. No se re-intenta ORS ni se pide key nueva. Reemplazado por **Valhalla** (instancia pública de OSM.de, sin API key), verificado en vivo contra Chile ANTES de escribir código: peatonal 15 min en Plaza Ñuñoa → Polygon de 34 vértices (~2,4 km), auto 10 min → 169 vértices (~5,2 km), ~1 s. Nuevo `lib/isocrona.ts` agnóstico de proveedor (valhalla | mapbox), nunca lanza, y ante cualquier fallo cae a círculo pero SIEMPRE marcado `metodo:'circulo_equivalente'` / `proveedor:null` — distinción de la que depende `calcularVeredictoCabida()` para degradar confianza a 'baja' y negarse a emitir veredicto. Las velocidades del círculo de respaldo son deliberadamente conservadoras (4,2 km/h a pie, 22 km/h auto) porque un radio en línea recta siempre sobreestima el área alcanzable. Conectado a `/api/demografia-consumo`, que ya no usa el radio fijo de 1km: el terreno de Ñuñoa pasó de 58.270 personas en 168 manzanas (círculo) a **60.094 en 156** (isócrona real) — menos manzanas pero más gente, porque la isócrona descarta las no alcanzables por calle y llega más lejos por los corredores viales. La geometría se omite del payload (hasta 169 vértices) porque la tarjeta solo necesita metadatos. 8 tests nuevos. **PENDIENTE (bloquea el veredicto)**: `cabida_comercial_cache` tiene 0 filas, y `calcularVeredictoCabida()` necesita percentiles p33/p66 calculados sobre análisis reales — hasta sembrar esa muestra con un batch sobre los 956 terrenos con coordenadas, el veredicto seguirá devolviendo `muestra_comparativa_insuficiente` por diseño (nunca inventa un corte). **PENDIENTE (manual)**: agregar `ISOCRONA_PROVEEDOR` y `MAPBOX_ACCESS_TOKEN` a `.env.local.example` — el hook de seguridad bloquea editar archivos `.env`. Sin setear nada funciona (default valhalla); Mapbox solo hace falta para batch masivo, porque la instancia pública de OSM.de es de uso comunitario razonable y no apta para eso.)
- (**Primer rédito del detector, 05-08.** (a) Falso positivo corregido en el propio `check-orphaned-exports.mjs`: excluía el archivo de definición al buscar usos, así que acusaba funciones que sí se usan internamente vía otra función exportada — `flagUnverifiedArticulo` es llamada por `flagUnverifiedCita` (misma línea 151) que alimenta 6 rutas de IA. Ahora se cuentan los usos dentro del propio archivo y esos se reclasifican como "solo uso interno — el export sobra" (informativo, no bloquea). **El baseline bajó de 39 a 15**: 24 de los 39 eran acusaciones falsas. Sin esta corrección el baseline habría acumulado ruido y el check se desacredita, que es la otra forma en que muere una herramienta así. (b) `getContextoLGUC` conectado a `/api/ai/copiloto`: `lib/lguc-knowledge.ts` existía hacía meses, escrito explícitamente con el mismo interface que OGUC "para reutilizar la recuperación por keywords", y con CERO callers — mientras `getContextoOGUC` estaba en 5+ rutas. O sea el copiloto conocía el reglamento (OGUC, D.S. 47) pero no la ley (LGUC, DFL 458). Era prioridad de founder declarada en la reunión del 30-jun. La consulta apunta a lo procedimental (permiso de edificación, recepción definitiva, responsabilidad, sanciones), que es lo que la LGUC cubre y la OGUC no; verificado que devuelve 4.714 chars con Art. 116 y Art. 144. **Con salvaguarda explícita**: 7 de los 9 artículos LGUC llevan `[VERIFICAR TEXTO OFICIAL]` y el módulo declara que son RESÚMENES, no transcripciones — el prompt ahora instruye al modelo a no presentarlos como texto legal literal ni entrecomillarlos, y a marcar que todo número/plazo con peso legal debe verificarse contra el DFL 458 oficial. Baseline 15 → 14.)
- (**Horizonte 1 del plan de producción, 05-08 — "wired or it doesn't count" + smoke E2E.** (1.1) `scripts/check-orphaned-exports.mjs` + `npm run check:orphans`, bloqueante en CI con trinquete: detecta exports de `lib/` sin ningún caller fuera de `tests/`. Nace de que el 04-08 aparecieron TRES módulos completos y testeados sin conectar; la causa fue de proceso, no de descuido — GSD encadena fases y cuando la 16 se bloqueó por el 403 de ORS, las fases 17-19 se escribieron igual porque el plan lo decía, midiendo "plan ejecutado" en vez de "el usuario puede verlo". **Solo bloquea funciones y clases, no tipos**: la primera corrida daba 175 hallazgos (mayoría `interface`/`type` inofensivos) y un reporte así se vuelve ruido y se ignora, que es como muere un check de calidad; filtrando a comportamiento quedan 39, y los tres huérfanos reales del 04-08 eran los tres funciones. Baseline congelado en esos 39 para no bloquear por deuda vieja. **Hallazgo del propio detector**: `lib/rag.ts` entero (búsqueda semántica sobre normativa: `generateEmbedding`, `matchNormativa`, `getContextoNormativoSemantico`), `lib/lguc-knowledge.ts` (`getContextoLGUC` — prioridad de founder de la reunión del 30-jun) y `lib/sii-lookup.ts` están desconectados; `lib/rag.ts:21` hasta tiene un comentario reconociéndolo. (1.2) `tests/e2e/rutas-detalle.smoke.spec.ts`: 7 smokes que afirman ausencia de 5xx Y de errores de consola, porque el 500 del 04-08 pasó `tsc --noEmit` y los 302 tests unitarios — TypeScript no puede expresar "este valor no sobrevive la serialización RSC". Validado reintroduciendo el bug exacto: typecheck y unit tests seguían en verde, el smoke falló. **La primera versión del smoke NO lo cazaba**: recorría los tabs con `if (count===0) continue` y solo afirmaba adentro del bucle, así que con la página en 500 no había tabs, las tres iteraciones hacían continue y el test pasaba sin evaluar una sola aserción — un smoke que puede pasar por no hacer nada es peor que no tenerlo. Corregido con aserción positiva previa al bucle. `playwright.config.ts`: `workers: 1` (con los 5 por defecto el `next dev` de un proceso se satura y tira ERR_CONNECTION_RESET, que parecen bugs de la app) y se omite `webServer` si viene `E2E_BASE_URL`, porque `next dev` no arranca si ya hay otro corriendo. De paso se arreglaron los 3 smokes preexistentes que estaban rojos hace tiempo (Chromium ni siquiera estaba instalado): dos por matcher de nombre no-exacto tras sumarse "Nuevo en Terrenos" al sidebar, y `expediente.smoke` porque clickeaba `/proyectos/zonificacion` creyendo que era un proyecto — la ficha estaba sana, el test nunca llegaba a ella. Suite E2E: 16/16.)
- (**Motor de cabida operativo 04-08** — continuación del desbloqueo de Fase 16. Faltaban las piezas que los Planes 16-04/16-05/19-03 nunca ejecutaron: `lib/cabida-comercial-server.ts` (no existía) y `/api/cabida-comercial/analisis` (no existía); el helper client-safe `consultarCabidaComercial` llevaba meses sin ningún caller. Nuevo `obtenerAnalisisCabidaComercial()` compone isócrona + población + consumo + competencia y persiste; `calcularPercentilesGapScore(formato)` calcula p33/p66 sobre análisis reales. **Decisión de esquema**: la migración aditiva NO puso todo en `cabida_comercial_cache` como anticipaba el comentario original — isócrona y población dependen de `(lat,lng,modo,minutos)` pero competencia y gap_score dependen del FORMATO, así que meterlos en la misma tabla los haría colisionar entre supermercado y minimarket. Es el mismo bug que se corrigió esa misma mañana en `mercado_locales_listings` (índice único sin `tipo_propiedad`). Por eso `poblacion_*` va en la tabla existente y la parte por formato en `cabida_comercial_competencia` con clave `(cache_id, formato)`. Ruta `/api/cabida-comercial/sembrar` idempotente (saltea ubicaciones ya analizadas, secuencial a propósito para no repetir el patrón de abuso que deshabilitó ORS). **Primera corrida real: 17 análisis de supermercado, muestraN=17 > MUESTRA_MINIMA=10 → el veredicto YA puede concluir** (verificado con test sobre los números reales: Ñuñoa, 8 competidores / 60.094 hab = gapScore 0,133 > p66 0,067 → `mercado_parece_cubierto`, primer veredicto concluyente del proyecto). **HALLAZGO INCÓMODO a resolver antes de mostrarlo en UI**: 9 de las 17 filas dieron CERO competidores y `confianzaGlobal='baja'`, y las 17 declaran `cobertura_conocida=false` (OSM + seed list no cubren Unimarc entre otros). O sea solo 8 de 17 ubicaciones pueden emitir veredicto, y el p33 quedó degenerado en 0 — la banda "evidencia_de_espacio" hoy equivale a "cero competidores detectados", que con fuente incompleta NO es lo mismo que "hay espacio". El veredicto ya lo trata correctamente como `confianza_degradada` (hay test que lo fija), pero antes de wirear la UI (Plan 19-04) conviene mejorar la cobertura de competencia — sembrar más ubicaciones NO lo arregla, es un problema de fuente, no de muestra.)
- (Nuevo 04-08 (mismo día): dos rediseños visuales más en la misma línea. (1) "Señales de ubicación" en `/terrenos/[id]` — de texto plano a `Badge` (variant default/muted según `cerca_avenida_principal`) + `KpiCard` para el conteo de anchors comerciales, sin badge "Verificado" a propósito porque la fuente (Overpass) ya se declara como potencialmente incompleta. (2) tab Posicionamiento de oportunidades — el grid estático de 3 cajas P25/Mediana/P75 se reemplazó por un `RankingBarChart` con "Este aviso" como cuarta barra resaltada, para ver de un vistazo dónde cae el precio en la cohorte en vez de comparar 3 números a mano. Ver la entrada de deuda técnica arriba para el bug real (crash 500 + gauge "0 UF" fabricado) encontrado y corregido durante este mismo trabajo. Verificado en vivo: terreno de Ñuñoa (7 anchors, badge "Cerca de avenida"), oportunidad de Maipú con precio inválido (fallback de texto, sin bug de 0 UF) y oportunidad de Maipú con precio válido (gráfico con "Este aviso" resaltado) — 0 errores de consola en los 3 casos.)
- **Post-v1.6**: `gsd-tools.cjs phase complete` tiene un bug de regex que captura la línea de Requirements equivocada cuando el bullet-summary de "Phases" en ROADMAP.md menciona una fase antes de la sección completa de otra — ocurrió en Fase 14 y 15, corregido a mano ambas veces, causa raíz no arreglada en el script. `gsd-tools.cjs milestone complete` también copió ROADMAP.md/REQUIREMENTS.md completos sin acotar al milestone (a diferencia de v1.4) — corregido a mano reconstruyendo `milestones/v1.6-*.md`. `.planning/STATE.md` sigue en formato narrativo legacy que los comandos `state advance-plan/update-progress` no pueden parsear — cada ejecutor de plan lo actualiza a mano.

## Constraints

- **TypeScript**: `any` prohibido — strict mode siempre
- **Auth**: Supabase Auth con RLS — multi-tenant por diseño
- **Secrets**: Nunca hardcodeados — variables de entorno siempre
- **Turbopack bug**: Path `/Estefanía/` tiene caracteres acentuados — evitar dashes en nombres de carpetas de rutas
- **Stripe**: Keys en `.env.local` + Vercel (usuario las configura manualmente)

## Key Decisions

| Decision | Rationale | Outcome |
|----------|-----------|---------|
| Next.js App Router (no Pages) | Moderna, edge-compatible, RSC para performance | ✓ Good |
| Supabase (no custom auth) | Auth + DB + RLS en un servicio | ✓ Good |
| Claude Sonnet 4.6 (no 3.5) | Mejor comprensión de documentos técnicos PDF | ✓ Good |
| Turbopack dev (no webpack) | Builds más rápidos, aceptar bug con path acentuado | ⚠️ Bug workaround: evitar dashes en rutas |
| Middleware dev bypass | Dev local sin Supabase auth — prod siempre enforced | ✓ Good |
| Route: `/calculadora` no `/calculadora-derechos` | Turbopack unicode bug workaround | ✓ Good |
| Dashboard en `/dashboard` (v1.1) | Landing page pública en `/` | — Pending |
| Zona PRC como cita `'PRC'` local en `due-diligence.ts` (no extender `FuenteNormativa`) | `getArticuloById`/`getContextoNormativo`/`flagUnverifiedCita` no pueden resolver una fuente GIS en vivo per-parcela — extender el tipo curado obligaría a esas funciones a manejar una fuente que estructuralmente no pueden resolver | ✓ Good |
| Señal de compatibilidad de uso (IA, no determinista) mantenida estrictamente fuera de `recomendarVia()` | El motor de vía de tramitación es puro y tiene tests de determinismo (`toEqual` en llamadas repetidas) — mezclar una señal de IA ahí habría roto esa garantía | ✓ Good |
| Guard compuesto `zona_status==='encontrado' && zona_usos_disponibles===true` en toda la superficie de zonificación (nunca solo `zona_status`) | Ñuñoa tiene `encontrado` pero usos estructuralmente vacíos — tratar solo `zona_status` como suficiente citaría/compararía contra texto vacío | ✓ Good |
| Verificación de checkpoints humanos con browser real (Playwright) + `mcp__supabase__execute_sql` para armar escenarios de prueba, en vez de solo tsc/eslint | Cada fase de v1.4 tuvo al menos un checkpoint bloqueante; verificar en vivo encontró bugs reales que tsc/eslint no habrían atrapado (migración SII nunca aplicada, mapa no se refrescaba tras Actualizar, mojibake residual, etiqueta de comuna incorrecta) | ✓ Good |
| Absorber PROPRA·BI como módulo nativo "Mercado Inmobiliario" (no un producto separado ni un submenú del copiloto de permisos) | El founder describió la tesis "arquitectura es dibujo" como diferenciador — el mercado inmobiliario necesitaba su propia identidad de módulo con switcher, no quedar enterrado; evita scope creep del copiloto de permisos | ✓ Good |
| Copiloto de Mercado Inmobiliario usa function/tool-calling (OpenAI `tools`) en vez de generar respuestas libres o RAG | Ancla cada respuesta en las funciones ya verificadas de `mercado-locales-server.ts`/`instrumentos-ipt-server.ts` en vez de dejar que el modelo invente cifras — primer uso de este patrón en el codebase | ✓ Good |
| Bautizar la función de gobernanza de datos/decisiones "Torre de Control", no "PMO" | El producto ya usa "PMO" para `components/proyecto/pmo-panel.tsx` (panel de sugerencias por proyecto) — mismo término para ambas cosas habría generado confusión real en conversaciones de equipo | ✓ Good |
| `data-sources.yaml` vive en `.planning/` (no en la raíz del repo) y se valida con un script determinista sin LLM, no con un agente | Co-ubicado con el resto del sistema de operaciones ya existente; un diff estructural (registro vs. filesystem vs. vercel.json) no necesita juicio de un modelo — más rápido, determinista, sin costo de tokens | ✓ Good |
| Captura de decisiones vía git hook (automática) en vez de un ritual de fin de sesión | El milestone v1.5 completo (17 commits) quedó invisible a `.planning/` precisamente porque dependía de que alguien se acordara de actualizarlo bajo presión de tiempo — un hook no se puede olvidar, es un efecto secundario de commitear | ✓ Good |
| `InformeEjecutivo` (Torre de Control §6) extendido solo a Tasación y Due Diligence, NO a Predictor/Auditor/Compliance-check/Reportes de Mercado/Memoria Descriptiva | Los primeros dos son streaming de markdown libre (búsqueda web agéntica) sin estructura compartida — ahí el componente aporta. Los otros 4 ya devuelven JSON estructurado con UI a medida que ya expresa bottom-line-up-front + verificación por campo (score circular + veredicto en Auditor, riesgo global + mes óptimo en Predictor, badge "Dato verificado"/"Estimado" por KPI en Reportes) — forzar el componente genérico ahí sería un downgrade visual, no una mejora. Memoria Descriptiva es un documento formal (texto de memoria para la DOM), no un informe de decisión con veredicto — el framing "resumen ejecutivo + confianza" no aplica a su género. | ✓ Good |
| Acceso comercial a TocToc (único vendor real de precios de transacción efectiva en Chile) — diferido, no descartado | Founder (1 ago 2026, confirmado 2026-08-01): política actual es no pagar por ninguna aplicación/dato de terceros por el momento — no es específico de TocToc, aplica a cualquier fuente paga que surja en la investigación de mercado en curso | — Deferred |
| RUT operativo real de Unimarc — no seguir investigando vía agentes | Founder (1 ago 2026): cobertura parcial de SMU (Alvi + Super10, sin Unimarc) es aceptable — más tiempo de búsqueda automatizada no vale la pena; reabrir solo si aparece por otra vía. Ver `sii-nomina-sucursales-holdings-sin-tiendas` en `data-sources.yaml` | ✓ Good |
| Refresh de bases de conocimiento legal (LGUC/OGUC/DDU) en cadencia fija trimestral (`freshness_sla_days: 90`), no ad-hoc | Founder (1 ago 2026): prefiere un recordatorio automático y determinista (el validador ya existente avisa cuando vence) a decidir caso a caso cuándo revisar | ✓ Good |
| `evaluarOportunidad()` extraído como fuente única de verdad para scoring, vía TDD (v1.6) | Previene que lista y ficha de detalle diverjan en qué cuenta como "oportunidad" — el bug exacto que motivó el prerequisito técnico de la Fase 13 | ✓ Good |
| Comparables y comparación de oportunidades vía queries nuevas y directas, no reusando `obtenerOportunidadesMercadoLocales()` (v1.6) | Esa función descarta listings sin reasonCodes — un subconjunto autoseleccionado, no el universo real de comparación | ✓ Good |
| Resumen ejecutivo IA de Oportunidades vía `streamConContexto` sin herramienta de búsqueda web, no `streamConBusquedaWeb` (v1.6) | Oportunidades sí tiene datos reales de mercado — dejar que el modelo busque en la web arriesgaría inventar contexto no anclado a esos datos | ✓ Good |
| Defensa de COMPA-03 en dos capas — checkbox deshabilitado (UX) + validación server-side real en `/comparar` (v1.6) | La URL `?ids=` es alcanzable directo, sin pasar por el checkbox — la validación real tiene que vivir en el servidor | ✓ Good |
| Informe exportable vía `@media print` + `window.print()`, no jsPDF/html2canvas (v1.6) | Resuelve una contradicción interna del research de milestone entre sus propios documentos; ninguno de los requisitos pedía generación sin usuario presente | ✓ Good |
| Sin gráficos Recharts en la v1 del informe exportable (v1.6) | Comportamiento de impresión de `ResponsiveContainer` no verificado en ningún caso real del repo — se evita el riesgo completo usando `GaugeArc`/`DesviacionBar` (SVG plano) | ✓ Good |

## Previous Milestones

- **v1.3 Army of Skills** (shipped 2026-06-26) — Copiloto IA drawer (Permisos/Desarchivo/Patentes), verificación DOM diaria automática, enriquecimiento SII automático, resumen semanal con tip de IA.
- **v1.4 Zonificación** (shipped 2026-07-30) — Zonificación automática por dirección (ArcGIS MINVU/OCUC), mapa con confirmación visual, compatibilidad de uso IA de 3 estados, fallback manual, integración aditiva en vía de tramitación/due diligence/copiloto. Full detail: `.planning/milestones/1.4-ROADMAP.md`.
- **v1.5 Fusión PROPRA·BI** (shipped 2026-08-01) — Absorción completa de la suite de inteligencia de mercado inmobiliario PROPRA·BI como módulo nativo "Mercado Inmobiliario", módulo Terrenos, instrumentos IPT, checklist dinámico, fusión de permisohub-enterprise, rediseño UX/IA de separación de módulos. Full detail: `.planning/MILESTONES.md`.
- **v1.6 Reportes Profesionales de Oportunidades** (shipped 2026-08-02) — Dashboard de detalle por oportunidad (posicionamiento vs. cohorte, historial, señales, comparables, resumen ejecutivo IA), comparación lado a lado (2-5 oportunidades, prevención estructural de mezclar tipo/operación), informe exportable/imprimible vía `@media print` con disciplina de fechas de vigencia. Full detail: `.planning/milestones/v1.6-ROADMAP.md`.

---

## Current Milestone: v1.7 Cabida Comercial (Demografía y Consumo)

**Goal:** Determinar si hay demanda real ("cabida") para un nuevo supermercado, minimarket, strip center o power center en una oportunidad, cruzando datos públicos de demografía y consumo dentro de un radio de influencia (isócrona) con la competencia existente.

**Target features:**
- Tab "Cabida Comercial" en la ficha de detalle de oportunidad (`/oportunidades/[id]`), junto a posicionamiento/comparables/historial (v1.6) — interfaz diseñada para poder correrse standalone sobre cualquier dirección/comuna en un milestone futuro, no solo atada a una oportunidad ya cargada
- Cruce de demografía y gasto de consumo (fuentes públicas chilenas: INE, encuestas de presupuesto familiar, etc.) dentro del radio de influencia de la ubicación
- Mapeo de competencia existente por formato (supermercado, minimarket, strip center, power center) dentro de ese radio
- Señal de cabida por formato — nunca binaria sin evidencia, con nivel de confianza y fuente citada (misma disciplina "nunca fabricar/ocultar datos" de v1.5/v1.6)

**Alcance confirmado con la founder (2 ago 2026):** radio de influencia (isócrona), no nivel comuna — el founder priorizó precisión sobre disponibilidad garantizada de datos; investigación previa a Requirements para validar qué fuentes públicas chilenas realmente permiten ese nivel de granularidad antes de comprometer alcance.

---
*Last updated: 2026-08-02 — inicio del milestone v1.7 (Cabida Comercial) vía `/gsd:new-milestone`*
