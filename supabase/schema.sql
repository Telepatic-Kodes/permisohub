...
-- ----------------------------------------------------------------------------
-- boletas_servicios
-- Boletas de agua, electricidad y gas por local para cumplimiento regulatorio.
-- Requeridas en: Informe Sanitario SEREMI (Ficha 163) y Autorización Sanitaria
-- de Alimentos (Ficha 172).
-- ----------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_boletas_local_id  ON boletas_servicios(local_id);
CREATE INDEX IF NOT EXISTS idx_boletas_periodo    ON boletas_servicios(periodo);
CREATE INDEX IF NOT EXISTS idx_boletas_estado     ON boletas_servicios(estado);
...
CREATE POLICY "boletas_workspace_read" ON boletas_servicios
  ...
  USING (
    EXISTS (
      ...
      WHERE l.id = boletas_servicios.local_id AND wm.user_id = auth.uid()
    )
  );
