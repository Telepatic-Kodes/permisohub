 ...
 * INE termina ~2021 — esto es HISTÓRICO, no una señal de "hoy".
 */
export async function obtenerTendenciaConstruccionComuna(comuna: string): Promise<TendenciaConstruccionComuna | null> {
  ...
  // Filtra por la columna `comuna_normalizada` (precalculada en la ingesta),
  // no por traer todo y comparar en cliente: el cap server-side de PostgREST
  ...
  // real, encontrado al verificar en vivo con una consulta directa, no solo
  // en teoría). Con la columna indexada, el resultado es un puñado de filas
  // por comuna (~26), nunca cerca del límite.
  ...
  const filasComuna = data

  const anioMax = Math.max(...filasComuna.map((f) => f.anio))
  const sumaEnRango = (desde: number, hasta: number) =>
    filasComuna.filter((f) => f.anio >= desde && f.anio <= hasta).reduce((acc, f) => acc + f.superficie_total_m2, 0)
